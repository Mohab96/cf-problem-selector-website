...
function valid(handle) {
    ...
    // Make the API call here adn store it in 'response'
...
function add_handle(handle) {
    ...
    for (let i = 0; i < acc_handles.length; i++) {
        acc_handles[i].addEventListener('click',
            (element) => {
...
document.getElementsByClassName('add-handle-btn')[0].addEventListener('click',
    () => {
...
function validate_input() {
    ...
    if (from % 100 !== 0 || to % 100 !== 0 || from > to) {
        alert('Please Enter a valid ratings');
        return false;
    } else if (from > 3500 || from < 800 || to > 3500 || to < 800) {
        alert('Please Enter a valid rating boundaries (800 - 3500)');
        return false;
    } else if (problems_cnt < 1 || problems_cnt > 50) {
        alert('Enter a valid number of problems (1 - 50)');
        return false;
    } else return true;
}
